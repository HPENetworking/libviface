// vifacego it's a GO packet wrapper of the libviface library
// for managing and creating network interfaces.
package vifacego
...
func VifaceCreateViface(name string, tap bool, id int,
	...
	var status = int(C.viface_create_viface(nameValue, C.bool(tap), C.int(id), result))
